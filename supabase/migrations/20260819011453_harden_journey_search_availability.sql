-- CommuteConnect capacity-aware passenger journey search.
--
-- This migration hardens the existing search_journeys RPC after the
-- introduction of booking requests.
--
-- Search now derives remaining capacity from accepted bookings and
-- excludes journeys for which the authenticated passenger already has
-- a pending or accepted request.
--
-- Search is exposed through a controlled RPC rather than broadening
-- direct SELECT access to other drivers' journey or profile rows.
--
-- Matching rules for this MVP:
--   - authenticated caller
--   - open, future journeys only
--   - exclude caller's own journeys
--   - enough seats remain after accepted bookings
--   - exclude journeys with an existing active request by this passenger
--   - origin within 5 km
--   - destination within 5 km
--   - departure within +/- 60 minutes of preferred time
--   - return passenger-safe summary fields only
--
-- Exact stored coordinates and journey_private_details are never returned.

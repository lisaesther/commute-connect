...
function mapBookingError(message: string) {
  ...
  if (
    normalised.includes(
      "cannot request seats on your own journey",
    )
  ) {
  ...
  if (
    normalised.includes(
      "not enabled for requesting rides",
    )
  ) {
  ...
  if (
    normalised.includes(
      "journey is not available for booking",
    )
  ) {
    ...
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
        ...
        <p className="mt-2 text-sm leading-6 text-amber-900">
          You are requesting{" "}
          <strong>
            {seatLabel(requestedSeats)}
          </strong>{" "}
          on this journey. The request will be
